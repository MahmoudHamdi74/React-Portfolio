...
const LightHouseScoreCard = memo(function LightHouseScoreCard() {
  ...
  return (
    ...
    >
      <div
        className={`bg-linear-to-br from-amber-100/50 via-amber-50/30 to-amber-100/50 dark:bg-linear-to-br dark:from-purple-900/30 dark:via-gray-800/40 dark:to-purple-900/30 backdrop-blur-sm rounded-xl p-6 md:p-8 border border-amber-300 dark:border-purple-500/30 transition-all duration-300 w-full max-w-md mx-4 ${isHovered ? 'border-amber-400 shadow-lg shadow-amber-200/40 dark:border-purple-400/50 dark:shadow-lg dark:shadow-purple-500/20 scale-105' : ''}`}
      ...
      >
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center space-x-2">
            <Zap className="w-4 h-4 text-amber-500 dark:text-purple-400" aria-hidden="true" />
            <h3 className="text-gray-700 dark:text-white font-semibold text-sm">Lighthouse Score</h3>
          </div>
          <div className="text-xl font-bold text-amber-600 dark:text-green-400" aria-label="Overall Score">100</div>
        </div>
          ...
          {LIGHTHOUSE_SCORES.map((score) => (
            <div key={score.name} className="flex items-center space-x-2">
              ...
              <div className="min-w-0 flex-1">
                <div className="text-gray-700 dark:text-white text-xs font-medium truncate">
                  {score.name}
                </div>
                <div className="text-gray-900 dark:text-gray-400 text-xs" aria-label={`${score.name} score`}>
        ...
        <div className="flex items-center justify-between text-xs">
          ...
          <a
            ...
            className="flex items-center space-x-1 text-amber-600 hover:text-amber-500 dark:text-purple-400 dark:hover:text-purple-300 transition-colors"
